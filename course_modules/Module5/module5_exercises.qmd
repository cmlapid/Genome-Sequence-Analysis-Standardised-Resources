...
Now we are going to start the PacBio assembly using the [**canu**](https://canu.readthedocs.io/) program. It first corrects the reads and then uses the **Celera** assembler to merge the long reads into contigs. To see the contents of the directory, type:
...
Now we will start the assembly with canu.
...
Generally, the k-mer size has the biggest impact on assembly results. Let us make a few other assemblies for different k-mer sizes i.e. 41 or 55. Here is the example for k-mer length of 55.
...
Repeat this for assemblies made with other kmer sizes.
...
The **canu** pacbio assembly should have hopefully finished by now. You can check the log using **less**, i.e. `less canu_log.txt`.
...
`bwa mem -t2 canu-assembly/PB.contigs.fasta IT.Chr5_1.fastq IT.Chr5_2.fastq > canu.sam`

`samtools sort -@2 -o canu.sorted.bam canu.sam`

`bcftools mpileup -f canu-assembly/PB.contigs.fasta -Ou canu.sorted.bam | bcftools call -mv -Ov -o canu.vcf`

Do the same for *wtdbg.ctg.lay.fasta*:

`bwa index wtdbg.ctg.lay.fasta`

`samtools faidx wtdbg.ctg.lay.fasta`

`bwa mem -t2 wtdbg.ctg.lay.fasta IT.Chr5_1.fastq IT.Chr5_2.fastq > wtdbg.sam`

`samtools sort -@2 -o wtdbg.sorted.bam wtdbg.sam`

`samtools mpileup -f wtdbg.ctg.lay.fasta -Ou wtdbg.sorted.bam | bcftools call -mv -Ov -o wtdbg.vcf`

And now compare some basic statistics using **bcftools stats**.
...
`bcftools consensus -i 'QUAL>1 && (GT="AA" || GT="Aa")' -H LA -f canu-assembly/PB.contigs.fasta canu.vcf.gz > canu-assembly/PB.contigs.polished.fasta`
...
`bcftools consensus -i 'QUAL>1 && (GT="AA" || GT="Aa")' -H LA -f wtdbg.ctg.lay.fasta wtdbg.vcf.gz > wtdbg.contigs.polished.fasta`

Finally, align and call variants like before using the polished assemblies as the reference this time.
