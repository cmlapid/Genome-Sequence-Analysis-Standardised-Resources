...
When scientists sequence DNA from a person or organism using modern **next-generation sequencing (NGS)** technology, the result is a set of many shorter DNA sequences from that sample, called **reads**. These reads can number in the thousands or millions, and each one represents a small piece of the original genetic material. Reads can be produced from just one end of a DNA fragment, or from both ends; these are called **single-end** and **paired-end** reads respectively. Because raw sequencing reads are typically short, numerous, and unordered, it is initially impossible to infer any meaning out of them. They must first be compared to an appropriate reference, in a process called **read alignment** or **read mapping**.

![**Sequencing reads and reference genomes**](images/reference%20genome.png)

A **reference genome** is a digital version of a species’ DNA used as a standard for comparison. When scientists perform NGS, they compare the resulting reads to the appropriate reference to figure out where they come from and what information they contain. The reference is stored as a big file (like `GRCh38.fa` for humans) that contains the full DNA sequence, or genome, of that species.
...
**Long-read sequencing** technologies, such as those from **Oxford Nanopore Technologies (ONT)** and **Pacific Biosciences (PacBio)**, enable the sequencing of **large DNA fragments**, typically tens of kilobases in length, with some platforms producing **ultra-long reads** of 100 kb or longer. These are especially useful for resolving repetitive regions and structural variants, as they often span entire transposable elements that short reads cannot. However, long-read sequencing can introduce new challenges, depending on platform and chemistry. Older PacBio technologies, as well as current ones by ONT, can be much more error-prone than short-read technologies, and result in error rates up to 10%. This can make accurate alignment more difficult, and require specialized algorithms for effective analysis.

![](images/alignment%20challenges.png)

Structural variants, such as **deletions** and **inversions**, can cause challenges for short-read aligners like **BWA-MEM**. In the case of a deletion (left), reads spanning the missing region may fail to align properly, often resulting in soft-clipping, mismatches, or gaps in coverage. For an inversion (right), alignment is disrupted due to the reversed orientation of the sequence, leading to a dense cluster of mismatches and misaligned bases. Paired-end reads help resolve these difficulties: if one read maps to a unique region while its mate spans a variant, the paired information can indicate the correct placement for the read that is difficult to align. **Split-read alignment strategies** further improve detection by allowing reads that partially overlap a breakpoint to be divided and aligned to two distinct locations, enabling more precise identification of structural variants.

### **Seed-and-chain alignment**

To address the limitations of traditional seed-and-extend alignment, modern long-read aligners like **Minimap2** use a **seed-and-chain** strategy, also called **seed-chain-extend**: short matching regions between a read and the reference are first identified, and then chained together into a consistent order before detailed alignment (extension) is performed. This reduces the search space while allowing large gaps or rearrangements between chained regions. As a result, long-read aligners can map reads across structural variants and repetitive regions more effectively than short-read methods, providing the alignment patterns needed for downstream structural variant detection.

![**Short-read vs long-read mapping**](images/seed%20and%20chain.png){width="600"}

### The **Data production workflow**

![**Data production workflow**](images/data%20production%20workflow.png)

The **Data production workflow** begins with sequencing instruments generating raw **FASTQ files** from individual lanes or plexes. These reads are then aligned to a reference genome using tools like **BWA** or **SMALT**, producing **BAM files** that represent aligned read data. Next, the **BAM improvement** step enhances data quality through operations such as sorting, duplicate marking, and base quality recalibration. Improved BAMs are merged at the **library level**, combining data from multiple lanes of the same library. They are then merged at the **sample/platform level**, integrating multiple libraries or sequencing runs from the same sample.

### Library Duplicates

**Library duplicates** are identical DNA fragments that arise during the **PCR amplification step** of library preparation for short-read sequencing. These duplicates can inflate read depth and potentially lead to **false SNP calls**, as they appear to support variants more strongly than they should. In high-quality libraries, duplicate rates are typically low—less than 5%. To address this, reads are first aligned to a reference genome, and **duplicate read-pairs**—those that map to the same outer coordinates—are identified and either removed or marked using tools such as **Samtools (`rmdup`)** or **GATK’s `MarkDuplicates`**. While **PCR-free protocols** can minimize duplicates, they require more input DNA, making them less feasible in some experimental contexts. Proper handling of duplicates is crucial for ensuring accurate variant calling and data interpretation.
...
Next-generation sequencing (NGS) platforms like Illumina NextSeq can generate up to \~800 million reads or 160 Gbp per run, producing massive data volumes that are computationally intensive to process. Aligning a single lane of reads on one computer can take a long time, so **parallel computing** is used to speed up the workflow. One approach is to split the FASTQ files into smaller chunks (e.g., 1 Gbp), align them independently, each taking 15-30 minutes using modern aligners, and then merge the resulting BAM files using tools like `samtools merge`. Alternatively, users can leverage multiple CPU cores on a single machine by simply specifying the number of threads (e.g., `bwa mem -t <threads>`), thereby speeding up the process without file splitting.

Estimating **IT requirements** involves two main components: **compute**, and **storage**. Compute relates to the total CPU time needed to perform an analysis, which for read alignment can be approximately 20-40 CPU hours per Gbp of sequence data. Storage refers to the digital space required to hold all the sequencing-related files, accounting for raw reads, aligned BAM files, intermediate results, and backups; this can amount to approximately 4-5 bytes per base sequenced. For example, a NextSeq run producing \~160 Gbp could require roughly 3,000 - 6,000 CPU hours and \~600-800 GB of storage, depending on the analysis pipeline and hardware used.
